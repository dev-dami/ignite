...
pub fn run_preflight(
    ...
    let dep_check = if dep_count > warn_count {
        PreflightCheck {
            ...
            message: format!(
                "High dependency count ({}). Consider reducing for faster cold starts.",
                dep_count
            ),
    ...
    let base_mb = mem_config
        .and_then(|m| m.base_mb)
        .unwrap_or(DEFAULT_BASE_MEMORY_MB);
    let per_dep_mb = mem_config
        .and_then(|m| m.per_dependency_mb)
        .unwrap_or(DEFAULT_MEMORY_PER_DEP_MB);
    let warn_ratio = mem_config
        .and_then(|m| m.warn_ratio)
        .unwrap_or(DEFAULT_WARN_RATIO);
    let fail_ratio = mem_config
        .and_then(|m| m.fail_ratio)
    ...
    let mem_check = if configured_memory < fail_threshold {
        PreflightCheck {
            ...
            message: format!(
                "Configured memory {}MB may be insufficient. Estimated need: {}MB based on {} dependencies",
                configured_memory, estimated_need, dep_count
            ),
        ...
        PreflightCheck {
            ...
            status: PreflightStatus::Warn,
            message: format!(
                "Configured memory {}MB is close to estimated need of {}MB",
                configured_memory, estimated_need
            ),
            value: Some(serde_json::json!(configured_memory)),
        ...
        PreflightCheck {
            ...
            status: PreflightStatus::Pass,
            message: format!(
                "Configured memory {}MB exceeds estimated need of {}MB",
                configured_memory, estimated_need
            ),
            value: Some(serde_json::json!(configured_memory)),
            threshold: Some(serde_json::json!(warn_threshold)),
    ...
    let min_ms = timeout_config
        .and_then(|t| t.min_ms)
        .unwrap_or(DEFAULT_MIN_TIMEOUT_MS);
    let max_ms = timeout_config
        .and_then(|t| t.max_ms)
        .unwrap_or(DEFAULT_MAX_TIMEOUT_MS);
    let cold_start_buffer = timeout_config
        .and_then(|t| t.cold_start_buffer_ms)
        .unwrap_or(DEFAULT_COLD_START_BUFFER_MS);
    ...
    let timeout_check = if configured_timeout < min_ms {
        ...
        if (configured_timeout as u64) < estimated_limit {
            PreflightCheck {
                ...
                status: PreflightStatus::Warn,
                message: format!(
                    "Timeout {}ms may be too short. Last execution: {}ms + {}ms buffer = {}ms",
                    configured_timeout, last_exec, cold_start_buffer, estimated_limit
                ),
                value: Some(serde_json::json!(configured_timeout)),
            ...
            PreflightCheck {
                ...
                status: PreflightStatus::Pass,
                message: format!(
                    "Timeout {}ms is within acceptable range",
                    configured_timeout
                ),
                value: Some(serde_json::json!(configured_timeout)),
                threshold: Some(serde_json::json!(max_ms)),
