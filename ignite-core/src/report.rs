...
fn get_suggestion(check_name: &str, status: &PreflightStatus) -> String {
    match (check_name, status) {
        ("image-size", PreflightStatus::Fail) => {
            "Use a smaller base image or consider multi-stage builds".to_string()
        }
        ("image-size", PreflightStatus::Warn) => {
            "Consider optimizing your disk image by removing unused files".to_string()
        }
        ("memory-allocation", PreflightStatus::Fail) => {
            "Increase memoryMb in service.yaml or reduce dependencies".to_string()
        }
        ("memory-allocation", PreflightStatus::Warn) => {
            "Monitor memory usage during execution and adjust if needed".to_string()
        }
        ("dependency-count", PreflightStatus::Fail) => {
            "Remove unused dependencies and consider using lighter alternatives".to_string()
        }
        ("dependency-count", PreflightStatus::Warn) => {
            "Review dependencies for unused packages".to_string()
        }
        ("timeout-config", PreflightStatus::Fail) => {
            "Increase timeoutMs in service.yaml or optimize function execution time".to_string()
        }
        ("timeout-config", PreflightStatus::Warn) => {
            "Consider adding buffer time for cold starts".to_string()
        }
...
pub fn format_report_as_text(report: &ExecutionReport) -> String {
    ...
    if let Some(ref exec) = report.execution {
        ...
        lines.push(format!(
            "  Cold Start: {}",
            if exec.cold_start { "Yes" } else { "No" }
        ));
        ...
        if !exec.stdout.trim().is_empty() {
            ...
            let indent_stdout: Vec<String> = exec
                .stdout
                .trim()
                .lines()
                .map(|l| format!("  {}", l))
    ...
    lines.push(format!(
        "  {}{}{} Overall Status: {}",
        overall_color, overall_icon, reset, report.preflight.overall_status
    ));
    lines.push(String::new());
